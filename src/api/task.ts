...
export async function updateTaskPlanQty(
  ...
  return invokeCommand("update_task_plan_qty", { taskId, planQty });
}

/** 通用更新任务（名称、计划日期、计划数量） */
export async function updateTask(input: UpdateTaskInput): Promise<Task> {
  return invokeCommand("update_task", { input });
}

/** 删除任务（P2-3：返回被删任务所属 goal_id，供局部更新进度） */
export async function deleteTask(taskId: string): Promise<DeleteTaskResult> {
  return invokeCommand("delete_task", { taskId });
}
...
export async function listTasksByDateRange(
  ...
  return invokeCommand("list_tasks_by_date_range", { startDate, endDate });
}
...
export async function removeTaskDependency(
  ...
  return invokeCommand("remove_task_dependency", { taskId, dependsOnId });
}
...
export async function validateDependencyChain(
  ...
  return invokeCommand("validate_dependency_chain", { taskId, dependsOnId });
}
...
export async function listTaskDependencyRecords(
  ...
  return invokeCommand("list_task_dependency_records", { taskId });
}
